...
import proposals from '../components/Data';
...
const ProposalScreen = () => {
    ...
    const handleVote = (event) => {
        ...
        if (Object.keys(selectedUser).length === 0){
            ...
            if (formRef.current){
                signTransaction(selectedUser.address, proposal.user, formRef.current.amount.value).then((data) => {
                    enqueueSnackbar('Transaction Successful', {variant: "success"})
                }).catch((error) => {
                    console.log(error.message)
                    if (error.message.indexOf('overspend') !== -1){
                        enqueueSnackbar("Your user doesn't have sufficient funds.", {variant: "error"})
                    } else {
                            ...
                            <Card sx={{ mb: 2 }}>
                                <form method="POST" onSubmit={handleVote} ref={formRef}>
                                    ...
                                    <CardActions variant="contained" color="secondary">
                                        <Button size="large" color="primary"  variant="contained" type="submit" fullWidth disabled={selectedVote === null || selectedUser.address === undefined }>Vote</Button>
                                    </CardActions>
                                </form>
                            </Card>
                        }
